        ...
        for i, page in enumerate(reader.pages):
            try:
                ...
                if not page_text.strip():
                    print(f"Warning: Empty or non-text content on page {i+1} in {file_path}")
            except Exception as page_e:
                print(f"Error extracting text from page {i+1} in {file_path}: {str(page_e)}")
                text += f"\n[Error extracting text from page {i+1}]\n"
                
        # If we got no text at all, try a fallback method
        if not text.strip():
            print(f"Warning: No text extracted from {file_path}. Attempting fallback method.")
