...
def initialize_vector_store(documents: List[Document] = None):
    ...
    if os.path.exists(FAISS_INDEX_PATH) and os.listdir(FAISS_INDEX_PATH): # Check if directory is not empty
        ...
        print("No documents provided and no existing index. Initializing empty FAISS store.")
        ...
        print("Vector store not initialized as no documents were provided and no index exists.")
        return
